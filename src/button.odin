...
draw_button_group :: proc(
	using buttonGroup: ^ButtonGroup,
	x: i32,
	y: i32,
	ySpacing: i32 = SCREEN_HEIGHT / 8,
) {
	currentY := y
...
set_button_group_colours :: proc(
	buttonGroup: ^ButtonGroup,
	baseColour: sdl.Color = {255, 255, 255, 255},
	hoverColour: sdl.Color = {255, 0, 0, 255},
	pressedColour: sdl.Color = {127, 0, 0, 255},
) {
...
button_group_handle_mouse_up :: proc(using buttonGroup: ^ButtonGroup, event: ^sdl.Event) -> i32 {
	...
	if sdl.HasIntersection(&mouseRect, &buttonGroup.buttonTexts[active].rect) {
		change_text_colour(
			...
			&buttonGroup.buttonTexts[active],
			buttonGroup.hoverColour,
		)
