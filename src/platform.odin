...
create_platform :: proc(
	...
	if potentiallyMoving && rand.uint32() % 4 == 0 {
		...
		platform.movingFrom = {
			rand.float64_range(dimensions.x / 2, SCREEN_WIDTH - (dimensions.x / 2)),
			position.y,
		}
		platform.movingTo = {
			rand.float64_range(dimensions.x / 2, SCREEN_WIDTH - (dimensions.x / 2)),
...
update_platforms :: proc(deltaTime: f64) {
	for &platform in platforms {
		if platform.moving {
			...
			if platform.position.x < platform.movingFrom.x ||
			   platform.position.x > platform.movingTo.x {
...
draw_platform :: proc(renderer: ^sdl.Renderer, platform: ^Platform) {
	rect: sdl.Rect = {
		cast(i32)(platform.position.x - (platform.dimensions.x / 2)),
		cast(i32)(platform.position.y - (platform.dimensions.y / 2)),
		cast(i32)platform.dimensions.x,
		cast(i32)platform.dimensions.y,
	}
...
random_platform :: proc(renderer: ^sdl.Renderer) -> (platform: Platform) {
	...
	{
		rand.float64_range(SCREEN_WIDTH / 8, SCREEN_WIDTH / 4),
		rand.float64_range(SCREEN_HEIGHT / 32, SCREEN_HEIGHT / 24),
	},
	)

	if !platform.moving { 	// Moving platforms will have a random position assigned
		platform.position.x = rand.float64_range(
			platform.dimensions.x / 2,
			SCREEN_WIDTH - (platform.dimensions.x / 2),
		)
		platform.position.y = rand.float64_range(
			(-SCREEN_HEIGHT / 4) - (platform.dimensions.y / 2),
			-platform.dimensions.y / 2,
		)
	}
...
random_platform_on_screen :: proc(renderer: ^sdl.Renderer) -> (platform: Platform) {
	...
	platform.position.y = rand.float64_range(
		platform.dimensions.y / 2,
		(SCREEN_HEIGHT * 3 / 4) - (platform.dimensions.y / 2),
	)
...
platforms_collide :: proc(first: ^Platform, second: ^Platform) -> bool {
	return(
		first.position.x + (first.dimensions.x / 2) >=
			second.position.x - (second.dimensions.x / 2) &&
		first.position.x - (first.dimensions.x / 2) <=
			second.position.x + (second.dimensions.x / 2) &&
		first.position.y + (first.dimensions.y / 2) >=
			second.position.y - (second.dimensions.y / 2) &&
		first.position.y - (first.dimensions.y / 2) <=
			second.position.y + (second.dimensions.y / 2) \
	)
}
