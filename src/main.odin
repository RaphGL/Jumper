...
main :: proc() {
	...
	if openSuccess {
		...
		if lastSpaceIndex != -1 {
			value, convertSuccess := strconv.parse_u64(
				cast(string)highScoreData[:lastSpaceIndex],
				10,
			)
			if convertSuccess {
			...
			value, convertSuccess = strconv.parse_u64(
				cast(string)highScoreData[lastSpaceIndex + 1:],
				10,
			)
			if convertSuccess {
	...
	for running {
			...
			if currentState == .PlayingNormal || currentState == .PlayingContinuousScrolling {
				...
				if !update_player(&player, deltaTime) {
					...
					if currentScore > get_current_mode_high_score() {
						...
						scoreString = fmt.tprintf(
							"Score: {} (Highscore: {})",
							currentScore,
							get_current_mode_high_score(),
						)
					}

					gameOverScoreText^ = create_text(
						renderer,
						helpFont,
						strings.clone_to_cstring(scoreString),
					)
				}
			}
		}
		...
		currentScoreText := create_text(
			renderer,
			helpFont,
			strings.clone_to_cstring(
				fmt.tprintf("Score: {} (Highscore: {})", currentScore, highScore),
	...
	if !os.write_entire_file(
		HIGH_SCORE_FILEPATH,
		transmute([]u8)fmt.tprintf("{} {}", highScoreNormalMode, highScoreContinuousScrolling),
	) {
...
reset_game :: proc(renderer: ^sdl.Renderer, player: ^Player) {
	...
	player.position = {
		SCREEN_WIDTH / 2,
		platforms[0].position.y - (platforms[0].dimensions.y / 2) - (player.dimensions.y / 2),
	}
