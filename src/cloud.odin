...
init_random_cloud :: proc(renderer: ^sdl.Renderer, cloud: ^Cloud) {
	...
	cloud.texture = img.LoadTexture(
		renderer,
		CLOUD_TEXTURE_FILEPATHS[rand.uint32() % cast(u32)len(CLOUD_TEXTURE_FILEPATHS)],
	)
	...
	cloud.velocity.x =
		rand.float64_range(CLOUD_MIN_VELOCITY, CLOUD_MAX_VELOCITY) *
		cast(f64)(((rand.int31() % 2) * 2) - 1)
}
